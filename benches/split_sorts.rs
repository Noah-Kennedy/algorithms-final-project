...
fn make_lists() -> Vec<Vec<Num>> {
    ...
    SIZES.iter()
         .map(|&size| {
             let mut v: Vec<Num> = (0..100).cycle().take(size as usize).collect();
...
fn bench_n_2_sorts(c: &mut Criterion) {
    ...
    let mut n2_sorts = c.benchmark_group("O(n^2) Sorts");
...
fn bench_n_log_n_sorts(c: &mut Criterion) {
    ...
    let mut n_log_n_sorts = c.benchmark_group("O(nlogn) Sorts");
